...
For each unlock window (clean) we build a corrupted run from an ordinary window of the same episode, restore clean activations one (block, timestep, token-type) cell at a time (token types: frame, action, latents), and measure restoration $(\ell_{\mathrm{patched}}-\ell_{\mathrm{corrupt}})/(\ell_{\mathrm{clean}}-\ell_{\mathrm{corrupt}})$ over $n{=}24$ window pairs per concept, following activation-patching best practice in the corruption scheme and restoration metric~\cite{zhang2024patching}; related causal-mediation analyses localise behaviour-carrying directions in language models~\cite{vig2020causal,todd2024functionvectors}.
