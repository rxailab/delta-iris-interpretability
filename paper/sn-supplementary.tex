...
\begin{table}[t]
\centering
\caption{\textbf{Write drives behaviour conditional on frame re-derivation}, across three independently trained agents. For each agent we select the wood feature whose teacher-forced imagination ablation most suppresses wood \emph{generation} (the \emph{generative gate}; its rank among the agent's wood-decoding features by detector lift in parentheses) and re-run the behavioural cascade with it. ``wood'', ``tools'': within-horizon probability the \achname{collect\_wood} / wooden-tool ($\achname{make\_wood\_pickaxe}$, $\achname{make\_wood\_sword}$, $\achname{place\_table}$) detector codes are sampled, baseline$\to$gate-ablated. ``HUD$\Delta$'', ``world$\Delta$'': mean absolute pixel change ($/255$) of the decoded HUD inventory strip ($y\ge49$) versus the world region under the lesion (teacher-forced, $n{=}40$). The gate is the best-decoding feature only in seed~0; in seeds~1--2 it is a low-lift feature (decodability $\ne$ generative control). Behaviour cascades exactly when the lesion also blanks the HUD copy (HUD$\Delta\gtrsim3$); off-tree achievements and the random control are unchanged in every case. Imagined return is not interpreted under the gate lesion (off-distribution world-model artefact).}\label{stab:gate}
\begin{tabular}{lccccccc}
\toprule
agent & gate (lift-rank) & wood & tools & HUD$\Delta$ & world$\Delta$ & cascade \\
\midrule
seed~0 (audited) & $f_{187}$ (1) & $1.00\!\to\!0.03$ & $0.80\!\to\!0.78$ & $0.8$ & $9.7$ & no \\
seed~1 & $f_{255}$ (25) & $1.00\!\to\!0.23$ & $0.78\!\to\!0.10$ & $4.0$ & $12.0$ & yes \\
seed~2 & $f_{996}$ (9) & $1.00\!\to\!0.12$ & $0.72\!\to\!0.03$ & $3.0$ & $8.5$ & yes \\
\bottomrule
\end{tabular}
\end{table}
